    ...
    .factory('OpenFB', function ($rootScope, $q, $window, $http) {
        ...
        function logout(fbScope) {
            ...
            if ( !tokenStore['fbtoken'] ) {
                return error({error: 'AccessToken not set.'});
            }
            ...
            logoutWindow = window.open(FB_LOGOUT_URL + '?access_token=' + tokenStore['fbtoken'] + '&next=' + logoutRedirectUrl +
                '&response_type=token&display=popup&scope=' + fbScope, '_blank', 'location=no');
 
            // If the app is running in Cordova, listen to URL changes in the InAppBrowser until we get a URL with an access_token or an error
            logoutWindow.addEventListener('loadstart', function (event) {
                var url = event.url;
                if (url.indexOf("access_token=") > 0 || url.indexOf("error=") > 0) {
                    logoutWindow.close();
                    tokenStore['fbtoken'] = undefined;
                    ...
                    deferredLogout.reject('Error on FB PHP logout');
                }
            });
